...
# Define NN function
def make_NN(n_hidden, n_epoch, labelsdict, lr, device, model_name, trainloader, validloader, train_data, finetuning_mode):
    # Import pre-trained NN model 
    model = getattr(models, model_name)(pretrained=(finetuning_mode!=0))
    
    # Freeze parameters that we don't need to re-train 
    if finetuning_mode == 1:
